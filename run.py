...
class Queue:
    ...
    def add(self):
        ...
        res.write(f'{self.tasks[self.tail]},')
        self.tail = (self.tail + 1) % self.max_size

    def show(self):
        print(f'Задача №{self.tasks[self.tail]} в приоритете')

    def do(self):
        print(f'Задача №{self.tasks[self.head]} выполнена')
        res.write(f'{self.tasks[self.head]},')
...
while True:
    ...
    if cmd == "empty":
        ...
        print(f"Введена неверная команда: {cmd}")
